...
class Loop:
    ...
    def __process_rlist(self, rlist):
        for fd in rlist:
            if fd == self.udp:
                ...
                logging.info(f"Received {len(data)} bytes from {addr}: {hexlify(data)}")
                if self.validator:
    ...
    def __process_wlist(self, wlist):
        for fd in wlist:
            try:
                ...
                continue
            logging.info(f"Sending {len(data)} bytes to {addr}: {hexlify(data)}")
            fd.sendto(data, addr)

    def __process_xlist(self, xlist):
        for fd in xlist:
            logging.error(f"Error state on {fd}")
        # TODO anything else?
    ...
    def stop(self):
        logging.info("Interrupting network thread")
        self.interrupted.set()
        self.__wake_select()
        self.thread.join()
        self.udp.close()
        logging.info("Network thread stopped")
