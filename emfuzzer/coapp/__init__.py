...
class CoappMonitorResult(StrEnum):
    SUCCESS = auto()
    UNEXPECTED_MESSAGE_RECEIVED = auto()


class CoappMonitor(TypedSubTask[CoappMonitorResult]):
    ...
    def start(self) -> CoappMonitorResult | SubTask.StartedType:
    ...
    def finish(self) -> CoappMonitorResult:
        assert self._socket
        assert self._validator
        self._delay.wait()
        self._io.close(self._socket)
        return (
            CoappMonitorResult.SUCCESS
            if self._validator.unexpected_messages == 0
            else CoappMonitorResult.UNEXPECTED_MESSAGE_RECEIVED
        )

    def result_type(self) -> type[CoappMonitorResult]:
        return CoappMonitorResult
...
class CoappInjector(TypedInjectionSubTask[Validator.Result]):
    def __init__(self, name: str, monitor: CoappMonitor):
        super().__init__(name)
    ...
    def from_config(cls, name: str, config: Config, context: Context) -> Self:
        return cls(
            name=name,
            monitor=context.data(CoappMonitor, config.get_str("monitor")),
        )
